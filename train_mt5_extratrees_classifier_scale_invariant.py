...
def make_classifier(random_state: int = 42) -> ExtraTreesClassifier:
...
def class_index_map(model: ExtraTreesClassifier) -> Dict[int, int]:
...
def write_run_in_mt5(
    ...
    txt = f"""MODEL: ExtraTreesClassifier (3 classes)
SYMBOL: {args.symbol}
TIMEFRAME: {args.timeframe}
...
STEPS:
1. Copy the ml_strategy_classifier_extratrees.onnx file next to the .mq5 EA.
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Train an ExtraTrees model for MT5 and export ONNX.")
    ...
    p.add_argument("--output-dir", default="output_extratrees", help="Output directory")
...
def main() -> None:
    ...
    onnx_path = output_dir / "ml_strategy_classifier_extratrees.onnx"
